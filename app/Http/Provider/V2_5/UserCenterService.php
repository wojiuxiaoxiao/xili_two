...
class UserCenterService extends CommonService
{
    ...
    public function answer_c_list($input_arr){
        $collect_list = CollectMulti::where([['interlocution.violate','=',1],['interlocution.status','=',1],['collect_multi.status','=',1],['collect_multi.user_id','=',$input_arr['userid']],['collect_multi.type','=',$input_arr['type']],['interlocution_group.status','=',1],['interlocution_group.shelves','=',1]])
            ->select('user.avatar','user.nickname','user.active','user.type as user_type','user.id as user_id','interlocution.id as interlocut_id','interlocution.anonymous','interlocution.title','interlocution.content','interlocution.answer_nums','interlocution.views','collect_multi.type')
        ...
        foreach($collect_list as $k=>$v){
            ...
            $collect_list[$k]['type'] = $input_arr['type'];
            $collect_list[$k]['lv'] = $this->getLv(intval($v['active']));
            $collect_list[$k]['views'] = testMillion($collect_list[$k]['views']);
        }
    ...
    public function _answerList($input)
    {
        ...
        if ($lists) {
            foreach ($lists as &$list) {
                ...
                $list['title'] = $this->subTitle($list['title']);
                $list['lv'] = $this->getLv(intval($list['active']));
                $list['content'] = $this->entityDecode($list['content']);//对输出文本进行反转义
...
//        $lists['is_own'] = $this->is_own($input['user_id']);//是否本人 1-是 0-不是
    ...
    public function bountryAnswerList($input){   
        ...
        $lists = Answer::where($p_where)
            ...
            ->select(
                ...
                'comment_interlocution.id',
                'interlocution.title',
                'interlocution.create_time as price',
                'comment_interlocution.content',
                'comment_interlocution.likes',
                'comment_interlocution.create_time',
                'comment_interlocution.user_id',
        ...
        if ($lists) {
            foreach ($lists as &$list) {
                $list['price'] = $list['price']>10000000 ? "" : number_format($list['price']/100,2);
                $list['likes'] = $this->testMillion($list['likes']);
                $list['answer_nums'] = $this->testMillion($list['answer_nums']);
                $list['title'] = $this->subTitle($list['title']);
                $list['lv'] = $this->getLv(intval($list['active']));
        ...
        $userInfo = $this->getUserInfoInUserCenter($input['user_id'],$input['login_status']);//获取头部的用户信息
        $user_share = $userInfo['share'] ? $userInfo['share'] : null;
        unset($userInfo['share']);
        return [
            'status' => 1,//即使没值返回空数组，状态仍为1
            'data' => $lists,
            'user' =>$userInfo,
    ...
    private function getUserInfoInUserCenter($user_id,$login_status=1)
    {
        $user = User::where('id', $user_id)
                    ->where('status', 1)
                    ->select('avatar', 'id', 'type as user_type', 'nickname','active', 'signature')
                    ->first();

        if($user['user_type']==2){
            $fans =  Focus::where('master_id',$user_id)->count();
            $user['fans'] = $fans ? $fans : 0;
            $user['master_desc'] = SignedAuthor::where('user_id',$user_id)->value('detail');
            $user['share'] = [
                'pic'=> $user['avatar'],
                'name'=>$user['nickname'],
                'summary'=>$user['master_desc'],
                'url'=>config('yunshui.http_url').'/share?share_id='.$user['id'].'&type=7',
            ];
        }

        $user['is_focus'] = 0;
        $user['lv'] = $this->getLv(intval($user['active']));
        $own_status = $this->is_own($user_id);
        if(!$own_status && $user['user_type']==2 && $login_status>0){    
            $focus_info  = Focus::where([['master_id','=',$user_id],['user_id','=', USERID]])->first();
    ...
    public function myGrowth($user_id){
        ...
        if($user){
            ...
            $day_diff = ($today_start-$sign_time_start)/(3600*24) +1;
    ...
    public function myFans($input_arr){
        $fans = Focus::where('master_id', $input_arr['master_id'])
            ->select('user.id as user_id','user.avatar','user.nickname','user.signature','user.type as user_type','user.active')
        ...
        foreach($fans as $k=>$v) {
            $fans[$k]['lv'] = $this->getLv(intval($v['active']));
        }
    ...
    public function signedAuthorBountrys($input_arr){
        ...
        {
            case 1:
                $where = [['comment_interlocution_bounty.status','=',1],['interlocution_bounty.violate','=',1],['comment_interlocution_bounty.user_id','=',$input_arr['user_id']],['interlocution_group.shelves','=',1]];
                $whereColumn = [['interlocution_bounty.comment_id','!=','comment_interlocution_bounty.id']];
                break;
            case 2:
                $where = [['comment_interlocution_bounty.status','=',1],['interlocution_bounty.violate','=',1],['interlocution_bounty.comment_id','>',0],['comment_interlocution_bounty.user_id','=',$input_arr['user_id']],['interlocution_group.shelves','=',1]];
                $whereColumn = [['interlocution_bounty.comment_id','=','comment_interlocution_bounty.id']];
                break;
        }

        $bountry_list = Commentib::where($where)->whereColumn($whereColumn)
            ->select('interlocution_bounty.title','interlocution_bounty.price','comment_interlocution_bounty.content','comment_interlocution_bounty.likes','interlocution_bounty.id as interlocut_id')
            ->leftJoin('interlocution_group', 'interlocution_group.id', '=', 'comment_interlocution_bounty.group_id')
    ...
    public function myFocusSaList($input_arr){
        ...
        foreach($focus_list as $k=>$v){
            ...
            $focus_list[$k]['user_type'] = $user_info['type'];
