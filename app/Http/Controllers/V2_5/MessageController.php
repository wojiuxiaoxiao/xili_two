...
class MessageController extends Controller
{
    ...
    public function systemNotice(){   
        ...
        if($userid){
            ...
            $list = FeedReply::where($where)
                             ->orWhere($whereIn)
                             ->select('feed_reply.content','feed_reply.create_time','feed_back.id')
    ...
    public function programMessage(){
        ...
        if($type==2){//如果是大师  直接评论观南的节目 和 回复评论观南的评论  （作者是观南 rootid=0  or  找出观南的评论id 评论的父级id是观南的评论id ）
            $comment_list = Comment::where([['comment.author_id','=',USERID],['rootid','=',0]])->orWhereIn('pid', Comment::where([['user_id','=',USERID]])->pluck('id'))
                ...
                ->select('user.id as user_id','user.avatar','comment.id','comment.user_nickname as nickname','comment.content','comment.rootid','comment.pid','comment.create_time','comment.type','program.name','program.id as program_id')
        ...
        $comment_list = Comment::whereIn('pid', Comment::where([['comment.user_id','=',USERID]])->pluck('id'))
            ->select('user.id as user_id','user.avatar','user.nickname','user.type as user_type','user.active','comment.id','comment.create_time','comment.rootid','comment.content','comment.pid','comment.program_id','comment.type')
            ->leftJoin('user', 'user.id', '=', 'comment.user_id')
            ->orderBy('create_time', 'desc')
            ->get()->toArray();
        ...
        foreach($comment_list as $k=>$comment){//找出评论的父评论且是用户的评论
            ...
            $comment_list[$k]['father'] = $comment_father ? $comment_father->toArray() : array();
            $comment_list[$k]['info'] = $program_info ? $program_info->toArray() : null;
            $comment_list[$k]['lv'] = $this->getLv(intval($comment['active']));
    ...
    public function articleMessage(){
        ...
        $comment_list = CommentArticle::whereIn('pid', CommentArticle::where([['comment_article.user_id','=',$user_id]])->pluck('id'))
            ->select('user.id as user_id','user.avatar','user.nickname','user.type as user_type','user.active','comment_article.id','comment_article.create_time','comment_article.rootid','comment_article.content','comment_article.pid','comment_article.article_id','comment_article.type')
...
//            ->limit($pagesize)
        ...
        foreach($comment_list as $k=>$comment){//找出评论的父评论且是用户的评论
            ...
            $comment_father= CommentArticle::where([['id','=',$comment['pid']],['user_id','=',$user_id]])->select('content','nickname')->orderBy('create_time', 'desc')->first();
    ...
    public function activityMessage(){
        ...
        $comment_list = CommentActive::whereIn('pid', CommentActive::where([['comment_active.user_id','=',USERID]])->pluck('id'))
            ->select('user.id as user_id','user.avatar','user.nickname','comment_active.id','comment_active.create_time','comment_active.rootid','comment_active.content','comment_active.pid','comment_active.active_id','comment_active.type')
...
//            ->limit($pagesize)
    ...
    public function answerReplys(){
        ...
        $comment_list = CommentInterlocution::whereIn('pid', CommentInterlocution::where([['comment_interlocution.user_id','=',USERID]])->pluck('id'))
            ->where('comment_interlocution.rootid','<>',0)
            ->select('user.id as user_id','user.avatar','user.nickname','user.type as user_type','user.active','comment_interlocution.pid','comment_interlocution.interlocut_id','comment_interlocution.id','comment_interlocution.create_time','comment_interlocution.rootid','comment_interlocution.content','comment_interlocution.type')
            ->leftJoin('user', 'user.id', '=', 'comment_interlocution.user_id')
            ->orderBy('comment_interlocution.create_time', 'desc')
            ->offset($start)
            ->limit($pagesize)
            ->get()->toArray();
        ...
        foreach($comment_list as $k=>$comment){
            ...
            $comment_father= CommentInterlocution::where([['id','=',$comment['pid']],['user_id','=',USERID]])->select('content')->orderBy('create_time', 'desc')->first();
            $comment_list[$k]['father'] = $comment_father ? $comment_father->toArray() : array();
            $Interlocution_info = Interlocution::where([['id','=',$comment['interlocut_id']]])->select('id as interlocut_id','title')->first();
            $comment_list[$k]['info'] = $Interlocution_info ? $Interlocution_info : '';
            $comment_list[$k]['lv'] = $this->getLv(intval($comment['active']));
        }

        $tmp['data'] = $comment_list ? $comment_list : array();
        $tmp['status'] = 1;
        $tmp['msg'] = '';
        extjson($tmp);
    ...
    public function answerTopic(){
        ...
        $comment_list = CommentInterlocution::whereIn('interlocut_id', Interlocution::where([['interlocution.user_id','=',USERID]])->pluck('id'))
            ->where('comment_interlocution.rootid','=',0)
            ->select('user.id as user_id','user.avatar','user.nickname','user.type as user_type','user.active','comment_interlocution.interlocut_id','comment_interlocution.id','comment_interlocution.create_time','comment_interlocution.rootid','comment_interlocution.content','comment_interlocution.type')
            ->leftJoin('user', 'user.id', '=', 'comment_interlocution.user_id')
            ->orderBy('comment_interlocution.create_time', 'desc')
            ->offset($start)
            ->limit($pagesize)
    ...
    public function redHint(){
        ...
        $tmp['answer_tp_status'] = $res5 ? 1 : 0;
        $tmp['answer_rp_status'] = $res6 ? 1 : 0;
        $tmp['msg'] = '';
        $tmp['status'] = 1;
        extjson($tmp);
    }
    ...
    /**
     * 请空红点提示[评论相关]
     * @type 1:活动 2:文章 3:节目  5：问答
     * @answer_type 1:回复 2：回答
     * @author neekli
     * @since v2.5
     */
    public function clearRedHint(){
        $this->checkUser();
        $type = Input::get('type');
        $answer_type = Input::get('answer_type');
        ...
        switch($type){
            ...
            case 5:
                if($answer_type==2){
                    try {
                        CommentInterlocution::whereIn('interlocut_id', Interlocution::where([['interlocution.user_id','=',USERID]])->pluck('id'))
                            ->where('comment_interlocution.rootid','=',0)
                            ->update(['comment_interlocution.read_status' => 1]);
                        extOperate(1);
                    } catch (Exception $e) {
                ...
                if($answer_type==1){
                    try {
                        CommentInterlocution::whereIn('pid', CommentInterlocution::where([['comment_interlocution.user_id','=',USERID]])->pluck('id'))
                            ->where('comment_interlocution.rootid','<>',0)
                            ->update(['comment_interlocution.read_status' => 1]);
                        extOperate(1);
                    } catch (Exception $e) {
