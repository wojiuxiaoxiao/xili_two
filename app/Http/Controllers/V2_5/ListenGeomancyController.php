...
class ListenGeomancyController extends Controller
{
    ...
    public function playRadio(){
        ...
        $program_info = Program::where($where)
            ->select('id', 'radio_pic', 'radio_url', 'name', 'showup_time',
                'burning_time', 'play_nums', 'column_name', 'audio_s_introduce',
                'audio_introduce', 'status as del_status', 'type','share_img','share_title','share_intro')
            ->first();
        $program_info['play_nums'] = $this->testMillion($program_info['play_nums']);
//        //累加积分
//        if($this->userid>0){
//            $growth_service = new GrowthRuleService($this->userid,'listen');
//            $growth_service->init();
//        }
        ...
        $collect = 0;
        if($this->userid>0){
            $collect = Collect::where([['status','=',1],['user_id','=',$this->userid],
                ['program_id','=',$program_id]])->value('status');
        }

        $good_list = Goods::where([['status','=',1],['program_id','=',$program_info['id']]])
            ->select('id','name','taobao_url','pic')->get();

        $program_info['state'] =  $collect ? 1 : 0;
        $program_info['goods'] =  $good_list;
        $summary = $program_info['name'] ? '#十方云水#快来听《'.$program_info['column_name'].'》的节目'.'“'.$program_info['name'].'”'
            : '我们专注于传递中国传统文化，弘扬正统易学之道，品味国学经典';
    ...
    public function collectProgram(){
        ...
        $program_info = Program::where([['status','=',1],['id','=',$program_id],['type','<>',2]])
        ...
        if($user_info){
            $update_res = Collect::where([['program_id','=',$program_id],
                ['user_id','=',$user_info['user_id']]])->update(['status' => 1]);
            extOperate($update_res,'收藏失败');
        }

        if(!$user_info){
            $insert_res = Collect::insert(['user_id' => USERID,'program_id'=>$program_id ,
                'program_name'=>$program_info['name'],'status' => 1,'create_time'=>time()]);
