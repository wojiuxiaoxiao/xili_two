    ...
    public function ArAcInof(){    
        ...
        if($type==1){//活动
            $return_info = Active::where($where)
                ->select('id','pic','title','update_time','content','imgsJson','views','likes','status','comment_nums as pcomments')
                ->first();
            ...
            $return_info = Article::where($where)
                ->select('id','pic','title','author','update_time','content','imgsJson','views','likes','status','comment_nums as pcomments')
                ->first();
        ...
        $share = $return_info['pic'] ? [
            'pic'=>$return_info['pic'],
            'name'=>$return_info['title'],
            'summary'=>cutstr_html(html_entity_decode($return_info['content']),50),
