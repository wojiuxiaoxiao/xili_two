...
define('HOST',config('getui.HOST'));
define('APPKEY',config('getui.APPKEY'));
define('APPID',config('getui.APPID'));
define('MASTERSECRET',config('getui.MASTERSECRET'));
define('CID',config('getui.CID'));
define('DEVICETOKEN',config('getui.DEVICETOKEN'));
define('Alias',config('getui.Alias'));
define('Badge',config('getui.Badge'));
...
//单推接口案例
function pushMessageToSingle($push){
    $igt = new IGeTui(NULL,APPKEY,MASTERSECRET,false);
    $template = IGtTransmissionTemplateDemo($push);

    //个推信息体
    $message = new IGtSingleMessage();

    $message->set_isOffline($push['isOffline']);//是否离线
    $message->set_offlineExpireTime($push['ExpireTime']);//离线时间
    $message->set_data($template);//设置推送消息类型
//	$message->set_PushNetWorkType(0);//设置是否根据WIFI推送消息，1为wifi推送，0为不限制推送
    //接收方
    $target = new IGtTarget();
    $target->set_appId(APPID);
    $target->set_clientId($push['clientId']);
//    $target->set_alias(Alias);


    try {
        $rep = $igt->pushMessageToSingle($message, $target);//result
        return $rep;
...
//群推接口案例
function pushMessageToApp($push){
    $igt = new IGeTui(HOST,APPKEY,MASTERSECRET);
    $template = IGtTransmissionTemplateDemo($push);
    //$template = IGtLinkTemplateDemo();
    //个推信息体
    ...
    $message->set_isOffline($push['isOffline']);
    $message->set_offlineExpireTime($push['ExpireTime']);//离线时间单位为毫秒，例，两个小时离线为3600*1000*2
...
function IGtTransmissionTemplateDemo($push){
    $template =  new IGtTransmissionTemplate();
    $template->set_appId(APPID);//应用appid
    $template->set_appkey(APPKEY);//应用appkey
    $template->set_transmissionType(2);//透传消息类型
    $template->set_transmissionContent($push['content']);//透传内容
    //$template->set_duration(BEGINTIME,ENDTIME); //设置ANDROID客户端在此时间区间内展示消息
    //APN简单推送
...
//        IOS8.2 支持
    ...
    $apn->badge=Badge;
    $apn->sound="";
    $apn->add_customMsg("payload",$push['content']);
    $apn->contentAvailable=1;
    $apn->category="ACTIONABLE";
    $template->set_apnInfo($apn);
