...
@if (isset($body['status']) && 0 === $body['status'])
    <div class="sf-wrap">
        ...
        <span class="sf-span" class="sf-xwrap-remove-title">{{ $body['msg'] }}</span>
...
<div class="sf-xwrap">
    <div class="sf-article-con">
        <div class="sf-ask-userInfo">
            ...
            @elseif (false == $body['avatar'])
            <img src="{{ asset('img/avatar.png') }}">
            @else
            <img src="{{ $body['avatar'] }}">
            @endif
            <span>{{ $body['nickname'] }}</span>
        </div>
        <div class="sf-article-title sf-xfont40">
            {{ $body['title'] }}
        </div>

        <div class="sf-article-content">{!! $body['content'] !!}</div>
        @if (count($body['pic']) > 0)
        <div class="sf-ask-img zoom">
            @foreach($body['pic'] as $pic)
            <div>
                <img src="{{ $pic }}">
            </div>
            @endforeach
        ...
        @endif
        <div class="sf-ask-attach"><span>{{ date('Y-m-d', $body['create_time']) }}</span><span>{{ $body['views'] }}阅读</span></div>
    </div>
    <div class="sf-article-line"></div>
    <div class="sf-article-comment">
        @if (1 === $body['is_choice'])
            <div class="sf-article-label">精选回答（{{ $body['answers'] }}）</div>
        @else
            <div class="sf-article-label">全部回答（{{ $body['answers'] }}）</div>
        @endif
        @if (!empty($comment))
            <ul>
                @if (isset($comment[0]))
                    <li>
                        <div class="sf-xcol-box">
                            <div  class="sf-article-img" >
                                @if (false == $comment[0]['avatar'])
                                    <img class="comment-logo" src="{{ asset('img/avatar.png') }}">
                                @else
        ...
        @else
            ...
            <div class="tc sf-index-empty">还没有人回答哦，快来回答吧~</div>
        @endif
    </div>
</div>
@endif
